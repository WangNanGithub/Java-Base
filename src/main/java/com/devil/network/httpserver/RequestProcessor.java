package com.devil.network.httpserver;
			...
			if (method.equals("GET")) {
				...
				if (theFile.canRead()
					...
					String body = new StringBuilder("<HTML>\r\n")
							...
							.append("</BODY></HTML>\r\n").toString();
					if (version.startsWith("HTTP/")) { // send a MIME header
						sendHeader(out, "HTTP/1.0 404 File Not Found", "text/html; charset=utf-8",
								body.length());
					}
					out.write(body);
					out.flush();
				...
				String body = new StringBuilder("<HTML>\r\n")
						...
						.append("</BODY></HTML>\r\n").toString();
				if (version.startsWith("HTTP/")) { // send a MIME header
					sendHeader(out, "HTTP/1.0 501 Not Implemented", "text/html; charset=utf-8",
							body.length());
				}
				out.write(body);
				out.flush();
